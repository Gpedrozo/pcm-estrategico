...
function patch(file, oldStr, newStr, label) {
  ...
  // Normalize both content and pattern to LF for matching
  ...
  // Determine if file uses CRLF
  const isCRLF = content.includes('\r\n');
  const newStrForFile = isCRLF ? newStr.replace(/\r?\n/g, '\r\n') : newStr;
  const oldStrForFile = isCRLF ? oldStr.replace(/\r?\n/g, '\r\n') : oldStr;
  // Use the normalized approach: replace in LF then convert back
  let result = contentLF.replace(oldLF, newStr.replace(/\r\n/g, '\n'));
  if (isCRLF) {
...
// 4. useOwner2Portal.ts - add refetchInterval
patch(
...
// 5. Owner2.tsx - add ownerThreadEndRef state
patch(
...
// 6. Owner2.tsx - add scroll on ticket select
patch(
  'src/pages/Owner2.tsx',
...
// 7. Owner2.tsx - add scroll ref div in thread
patch(
  'src/pages/Owner2.tsx',
  `                                </div>
                              )

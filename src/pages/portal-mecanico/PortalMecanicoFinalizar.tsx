...
const CHIPS_ACAO = [
  'Troca de peça', 'Ajuste mecânico', 'Limpeza', 'Lubrificação',
  'Alinhamento', 'Soldagem', 'Calibração', 'Reparo elétrico',
];

const CHIPS_MATERIAIS = [
  'Rolamento', 'Correia', 'Parafuso', 'Graxa',
  'Óleo', 'Vedação', 'Filtro', 'Mangueira',
];

interface FotoItem { url: string; name: string; fase: string; timestamp: number; }

export default function PortalMecanicoFinalizar() {
  ...
  const handleChipAcao = (chip: string) => {
    setObservacoes(prev => prev.includes(chip) ? prev : prev ? `${prev}, ${chip}` : chip);
  };

  const handleChipMaterial = (chip: string) => {
    setMateriaisUsados(prev => prev.includes(chip) ? prev : prev ? `${prev}, ${chip}` : chip);
  };
  ...
  const handleFotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (error) {
      ...
      setFotosDepois(prev => [...prev, { url: urlData.publicUrl, name: file.name, fase: 'depois', timestamp: Date.now() }]);
  ...
  const handleFinalizar = () => {
    if (!os) return;
    if (!observacoes.trim()) {
      toast({ title: 'Observação obrigatória', description: 'Descreva o que foi feito antes de finalizar.', variant: 'destructive' });
      return;
    }
  ...
  return (
    <div className="space-y-5 py-6 pb-8 md:pt-16">
      ...
      <div className="flex items-center gap-3">
        <button onClick={() => navigate(-1)} className="min-w-[48px] min-h-[48px] flex items-center justify-center rounded-2xl border-2 hover:bg-muted active:scale-90 transition-all">
          ...
          {CHIPS_ACAO.map(chip => (
            <button
              ...
              className={cn(
                ...
                observacoes.includes(chip) ? 'border-orange-500 bg-orange-500/10 text-orange-700' : 'border-border bg-card'
              )}
            >
              {chip}
            </button>
        ...
        <div className="flex flex-wrap gap-2">
          {CHIPS_MATERIAIS.map(chip => (
            <button
              ...
              className={cn(
                'px-4 py-2.5 rounded-full border-2 text-sm font-semibold transition-all active:scale-95',
                materiaisUsados.includes(chip) ? 'border-orange-500 bg-orange-500/10 text-orange-700' : 'border-border bg-card'
              )}
            >
              {chip}
            </button>
          ...
          <div className="flex gap-2 overflow-x-auto pb-2">
            {fotosDepois.map((f, i) => (
              <div key={i} className="relative w-24 h-24 flex-shrink-0 rounded-xl overflow-hidden border-2">
                ...
                <button onClick={() => setFotosDepois(prev => prev.filter((_, idx) => idx !== i))} className="absolute top-1 right-1 bg-black/60 rounded-full p-0.5">
      ...
      <button
        ...
        className={cn(
          ...
          equipamentoOperando ? 'border-green-400 bg-green-500/10' : 'border-red-400 bg-red-500/10'
        )}
      >
        {equipamentoOperando
        ...
        <div className="text-left">
          ...
          <p className={cn('text-sm font-semibold', equipamentoOperando ? 'text-green-700 dark:text-green-400' : 'text-red-700 dark:text-red-400')}>
      ...
      {(state?.fotos || []).length > 0 && (
        <div className="space-y-2">
          <h3 className="text-sm font-bold text-muted-foreground">Fotos capturadas (antes/durante)</h3>
          <div className="flex gap-2 overflow-x-auto pb-2">
            {(state?.fotos || []).map((f, i) => (
      ...
      {/* Botão Finalizar */}
      <Button
        className="w-full h-20 text-xl font-black gap-3 rounded-2xl active:scale-95 transition-all shadow-xl bg-green-600 hover:bg-green-700"
        onClick={handleFinalizar}
        disabled={updateOS.isPending}
      >
        {updateOS.isPending ? <Loader2 className="h-7 w-7 animate-spin" /> : <CheckCircle2 className="h-7 w-7" />}
        FINALIZAR O.S.
      </Button>
    </div>
  );

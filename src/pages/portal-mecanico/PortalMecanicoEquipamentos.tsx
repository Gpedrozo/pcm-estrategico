...
import {
  ArrowLeft, Search, MapPin, Factory, Hash,
  Calendar, AlertTriangle, Wrench, Tag, Info,
} from 'lucide-react';
...
export default function PortalMecanicoEquipamentos() {
  ...
  const osDoEquip = equip
    ? (ordens || []).filter(o => o.tag === equip.tag).sort((a, b) =>
        new Date(b.data_solicitacao || b.created_at || '').getTime() - new Date(a.data_solicitacao || a.created_at || '').getTime()
      ).slice(0, 10)
    : [];
    ...
    return (
      <div className="space-y-5 py-6 pb-8 md:pt-16">
        <div className="flex items-center gap-3">
          <button onClick={() => { setSelectedId(null); setSearch(''); }} className="min-w-[48px] min-h-[48px] flex items-center justify-center rounded-2xl border-2 hover:bg-muted active:scale-90 transition-all">
    ...
    <div className="space-y-5 py-6 pb-8 md:pt-16">
      ...
      <div className="space-y-2">
        <div className="relative">
          ...
          <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar por TAG ou nome..." className="pl-11 h-14 text-base rounded-2xl border-2" autoFocus />
        </div>

        {filtrados.length > 0 && (
          <div className="border-2 rounded-2xl overflow-hidden divide-y max-h-[60vh] overflow-y-auto">
            {filtrados.map(e => (
              <button key={e.id} onClick={() => setSelectedId(e.id)} className="w-full p-4 text-left hover:bg-muted active:bg-muted/80 transition-colors">
                <div className="flex items-center justify-between">
                  <div>

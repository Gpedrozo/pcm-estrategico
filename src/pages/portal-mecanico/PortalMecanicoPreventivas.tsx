...
import {
  ArrowLeft, Calendar, Clock, Wrench, Filter,
} from 'lucide-react';
...
export default function PortalMecanicoPreventivas() {
  ...
  const preventivas = useMemo(() => {
    return (ordens || []).filter(o => {
      ...
      if (mecanico && o.mecanico_responsavel_id && o.mecanico_responsavel_id !== mecanico.id) return false;
        ...
        {([
          ...
          <button
            ...
            className={cn(
              ...
              filtro === f.val ? 'border-orange-500 bg-orange-500/10 text-orange-700' : 'border-border bg-card',
            )}
          >
        ...
        <div className="space-y-2">
          {preventivas.map(os => {
            ...
            return (
              <button
                key={os.id}
                onClick={() => isAberta ? navigate(`/portal-mecanico/os/${os.id}`) : undefined}
                disabled={!isAberta}
                className={cn(
              ...
              >
                ...
                <div className="flex items-center gap-3 mt-2 text-xs text-muted-foreground">
                  {dataStr && <span className="flex items-center gap-1"><Calendar className="h-3 w-3" /> {dataStr}</span>}
                  {os.tempo_estimado && <span className="flex items-center gap-1"><Clock className="h-3 w-3" /> {os.tempo_estimado}</span>}
                  <span className="flex items-center gap-1"><Wrench className="h-3 w-3" /> PREVENTIVA</span>
                </div>
              </button>
            );
          })}

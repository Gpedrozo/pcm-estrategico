...
export default function CriarOSScreen() {
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      // numero_os is auto-generated by BEFORE INSERT trigger (per-tenant sequential)
      const { data: insertedOS, error } = await supabase.from('ordens_servico').insert({
      ...
      }).select('numero_os').single();

      if (error) throw error;

      const createdNum = insertedOS?.numero_os || 0;
      ...
      resetForm();
      writeAuditLog({ action: 'CREATE_OS_MOBILE', table: 'ordens_servico', empresaId, source: 'CriarOSScreenV2', metadata: { numero_os: createdNum, tipo, prioridade } });
      showSuccess(`O.S. #${String(createdNum).padStart(4, '0')} criada com sucesso!`, () => nav.goBack());
    } catch (err: any) {
      showError(err);
    } finally {
      setSaving(false);

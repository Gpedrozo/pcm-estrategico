...
-- Smoke checks
SELECT
  to_regclass('public.audit_logs') AS audit_logs_ok,
  to_regclass('public.enterprise_audit_logs') AS enterprise_audit_logs_ok;

SELECT proname
FROM pg_proc
WHERE proname IN ('app_write_audit_log', 'get_current_empresa_id', 'has_permission')
ORDER BY proname;

SELECT tablename, policyname
FROM pg_policies
WHERE schemaname = 'public'
  AND tablename IN ('audit_logs', 'enterprise_audit_logs')

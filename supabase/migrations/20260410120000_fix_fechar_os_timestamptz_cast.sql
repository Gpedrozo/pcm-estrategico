-- ============================================================
-- FIX: "invalid input syntax for type timestamp with time zone: '12:10'"
--
-- Causa: Migration 20260404060000 alterou hora_inicio/hora_fim de TIME
-- para TIMESTAMPTZ em execucoes_os, mas a RPC close_os_with_execution_atomic
-- ainda faz p_hora_inicio::time / p_hora_fim::time no INSERT.
--
-- Correção: Usar v_inicio_ts / v_fim_ts (que já combinam data + hora)
-- no INSERT, pois essas variáveis já são timestamp completos.
-- ============================================================
...
begin
  ...
  -- ═══ FIX: usar v_inicio_ts / v_fim_ts em vez de p_hora_inicio::time ═══
  -- As colunas hora_inicio/hora_fim agora são TIMESTAMPTZ (migration 20260404060000)
  ...
  values (
    ...
    p_mecanico_nome,
    v_data_inicio,
    v_inicio_ts,    -- ← era p_hora_inicio::time (quebrava com TIMESTAMPTZ)
    v_fim_ts,       -- ← era p_hora_fim::time (quebrava com TIMESTAMPTZ)
    v_tempo_liquido,
  ...
  -- Check if descricao_execucao column exists (may have been added outside migrations)
  SELECT EXISTS (

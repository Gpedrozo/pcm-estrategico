-- Fix: constraint "check_descricao_execucao_required" on ordens_servico was created
-- outside of migrations and blocks the close_os_with_execution_atomic RPC.
-- The system stores execution description in execucoes_os.servico_executado, not
-- in ordens_servico.descricao_execucao. Drop the orphan constraint.
...
BEGIN
  IF EXISTS (
    ...
    WHERE table_schema = 'public'
      ...
      AND constraint_name = 'check_descricao_execucao_required'
  ) THEN
    ALTER TABLE public.ordens_servico
      DROP CONSTRAINT check_descricao_execucao_required;
    RAISE NOTICE 'Dropped constraint check_descricao_execucao_required';
  END IF;
END
$$;

-- Also update the close RPC to populate descricao_execucao column if it exists,
-- ensuring forward-compatibility regardless of column state.

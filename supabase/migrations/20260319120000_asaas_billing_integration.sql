...
BEGIN
  IF to_regclass('public.subscriptions') IS NOT NULL
    ...
    WHERE conname = 'subscriptions_billing_provider_check'
      AND conrelid = to_regclass('public.subscriptions')
  ) THEN
    ALTER TABLE public.subscriptions
...
BEGIN
  IF to_regclass('public.subscriptions') IS NOT NULL THEN
    EXECUTE 'CREATE UNIQUE INDEX IF NOT EXISTS idx_subscriptions_asaas_subscription_id
...
BEGIN
  IF to_regclass('public.subscriptions') IS NOT NULL THEN
    EXECUTE 'CREATE INDEX IF NOT EXISTS idx_subscriptions_billing_provider
...
BEGIN
  IF to_regclass('public.subscriptions') IS NOT NULL THEN
    EXECUTE 'CREATE INDEX IF NOT EXISTS idx_subscriptions_asaas_customer_id
...
BEGIN
  IF to_regclass('public.subscriptions') IS NOT NULL
    AND to_regclass('public.subscription_payments') IS NULL THEN
...
BEGIN
  IF to_regclass('public.subscription_payments') IS NOT NULL THEN
    EXECUTE 'CREATE UNIQUE INDEX IF NOT EXISTS idx_subscription_payments_provider_payment_id
...
BEGIN
  IF to_regclass('public.subscription_payments') IS NOT NULL THEN
    EXECUTE 'CREATE INDEX IF NOT EXISTS idx_subscription_payments_provider

...
DO $$
BEGIN
  -- Skip if status column does not exist in this environment
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
...
BEGIN
  IF EXISTS (
    ...
    EXECUTE 'CREATE INDEX IF NOT EXISTS idx_anomalias_inspecao_status ON public.anomalias_inspecao(empresa_id, status)';
  END IF;
END $$;

...
alter table if exists public.execucoes_os_pausas
  add column if not exists data_inicio date,
  add column if not exists data_fim date;
...
begin
  ...
  v_data_fim := coalesce(p_data_fim, v_data_inicio);

  if v_data_fim < v_data_inicio then
    raise exception 'invalid_execution_period';
  end if;

  v_inicio_ts := v_data_inicio + nullif(p_hora_inicio, '')::time;
  v_fim_ts := v_data_fim + nullif(p_hora_fim, '')::time;

  if v_inicio_ts is null or v_fim_ts is null or v_fim_ts <= v_inicio_ts then
    raise exception 'invalid_execution_window';
  end if;
  ...
  if p_pausas is not null and jsonb_typeof(p_pausas) = 'array' then
    ...
    loop
      begin
        ...
        v_pausa_data_inicio := coalesce(nullif(v_pausa->>'data_inicio', '')::date, v_data_inicio);
        v_pausa_data_fim := coalesce(nullif(v_pausa->>'data_fim', '')::date, v_pausa_data_inicio);

        if v_pausa_inicio is null or v_pausa_fim is null then
          continue;
        end if;

        v_pausa_inicio_ts := v_pausa_data_inicio + v_pausa_inicio;
        v_pausa_fim_ts := v_pausa_data_fim + v_pausa_fim;
...
$$;
  ...
  revoke execute on function public.close_os_with_execution_atomic(
    uuid, uuid, text, date, text, date, text, integer, text, numeric, numeric, numeric, numeric, jsonb, uuid, text, text, text, text, jsonb
  ) from anon;
